...
class BaseRoute
{
    ...
    {
        // Index
        \Route::get($name, ['as' => $name.'.index', 'uses' => $controller.'@index', $options, 'middleware' => 'auth:view']);

        // Store
        \Route::post($name, ['as' => $name.'.store', 'uses' => $controller.'@store', $options, 'middleware' => 'auth:create']);

        // Create
        \Route::get($name.'/create', ['as' => $name.'.create', 'uses' => $controller.'@create', $options, 'middleware' => 'auth:create']); // for viewing
        \Route::post($name.'/create', ['as' => $name.'.create', 'uses' => $controller.'@create', $options, 'middleware' => 'auth:create']);

        // update
        \Route::patch("$name/{{$name}}", ['as' => $name.'.update', 'uses' => $controller.'@update', $options, 'middleware' => 'auth:edit']);
        \Route::put("$name/{{$name}}", ['as' => $name.'.update', 'uses' => $controller.'@update', $options, 'middleware' => 'auth:edit']);

        // delete
        \Route::delete("$name/{{$name}}", ['as' => $name.'.destroy', 'uses' => $controller.'@destroy', $options, 'middleware' => 'auth:delete']);

        // edit
        //		\Route::get("$name/{{$name}}/edit", array('as' => $name.'.edit', 'uses' => $controller.'@edit', $options, 'middleware' => 'auth:edit'));
        \Route::get("$name/{{$name}}/edit", ['as' => $name.'.edit', 'uses' => $controller.'@edit', $options, 'middleware' => 'auth:view']);

        \Route::get("$name/{{$name}}/dump", ['as' => $name.'.dump', 'uses' => $controller.'@dump', $options, 'middleware' => 'auth:edit']);

        \Route::get("$name/dump", ['as' => $name.'.dumpall', 'uses' => $controller.'@dumpall', $options, 'middleware' => 'auth:edit']);

        // Fulltext Search
        // TODO: adapt route name to not strtolower() like other functions
        \Route::get(strtolower($name).'/fulltextSearch', ['as' => $name.'.fulltextSearch', 'uses' => $controller.'@fulltextSearch', $options, 'middleware' => 'auth:view']);

        // AJAX Index DataTable
        \Route::get("$name/datatables", ['as' => $name.'.data', 'uses' => $controller.'@index_datatables_ajax', $options, 'middleware' => 'auth:view']);
    }
